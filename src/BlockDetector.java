...
public class BlockDetector extends Thread {
	 ...
	 */
	public BlockDetector(SampleProvider colorSensor, float[] colorData, Navigation navi, Odometer odo, EV3LargeRegulatedMotor leftMotor, 
	...
	public void investigateBlock()
	{
		...
		if(difference <= 88 && difference >= 75)//within center range
		{
			Sound.buzz();
			Sound.buzz();
	...
	{
		//back up 
		while(USDistance <= 13)//arms dont hit at 21cm. perfect dist: 13cm
		{
			rightMotor.backward();
			leftMotor.backward();
			USDistance = getFilteredUSData();
		...
		//back up 
		rightMotor.rotate(convertDistance(WHEEL_RADIUS,-8),true);//arms dont hit at 15cm, perf dist: -8
